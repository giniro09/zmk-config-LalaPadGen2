...
#else
#define TAP_REPEAT_DIAG_COUNT 0
#define TAP_REPEAT_DIAG_INTERVAL_MS 0
#endif
...
#ifndef CONFIG_INPUT_IQS9151_TOUCH_POSITION_DIAG_RANGE
#define CONFIG_INPUT_IQS9151_TOUCH_POSITION_DIAG_RANGE 800
#endif
...
enum iqs9151_force_mode {
    IQS9151_FORCE_MODE_NONE = 0,
...
struct iqs9151_data {
    ...
    int64_t last_single_finger_motion_ms;
...
static bool iqs9151_update_gesture_sessions(struct iqs9151_data *data,
    ...
    if (!suppress_one_finger_gestures &&
        frame->finger_count == 3U && data->one_finger.active) {
    ...
    if (!suppress_one_finger_gestures &&
        frame->finger_count == 2U && data->one_finger.active) {
        const int64_t elapsed_ms = k_uptime_get() - data->one_finger.down_ms;
    ...
    if (!suppress_one_finger_gestures &&
        frame->finger_count != 1U && data->one_finger.active) {
    ...
    case 1U:
        if (!suppress_one_finger_gestures &&
            !(data->two_finger.active && data->two_finger.release_pending)) {
...
static void iqs9151_process_frame(struct iqs9151_data *data,
    ...
    if (IS_ENABLED(CONFIG_INPUT_IQS9151_TOUCH_POSITION_DIAG)) {
        iqs9151_report_touch_position_diag(dev, frame);
